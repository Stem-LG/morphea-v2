...
function StoreAssignmentModal({ user, stores, isOpen, onClose, onAssign, loading }: StoreAssignmentModalProps) {
    ...
    const handleNext = () => {
        if (currentStep === 1) {
            ...
            const isFirstTimeAssignment = !hasDesigner && selectedStoreIds.length > 0 && (user?.assigned_stores || []).length === 0;
...
export default function UsersManagementPage() {
    ...
    const canAssignStores = (user: UserRole) => {
        // Anonymous users (users with no roles or only "user" role) cannot assign stores
                                ...
                                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div className="flex space-x-2">
                                            ...
                                            {!user.roles.includes("admin") && canAssignStores(user) && (
                                                <button
                                            ...
                                            {!user.roles.includes("admin") && !canAssignStores(user) && (
                                                <span className="text-gray-400 dark:text-gray-500 text-xs">
                                                    {user.roles.length === 0 || (user.roles.length === 1 && user.roles[0] === "user")
                                                        ? t("admin.users.anonymousUser")
                                                        : t("admin.users.insufficientPermissions")}
                                                </span>
                                            )}
                                        </div>
                                    </td>
