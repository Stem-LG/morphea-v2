...
// Individual tree model component
function TreeModel({ url, name }: { url: string; name: string }) {
  console.log('TreeModel component loading:', { url, name });
...
export default function Tree3DViewer({ treeData, onClose }: Tree3DViewerProps) {
  return (
      ...
      <div className="relative w-full max-w-4xl h-[80vh] bg-gradient-to-br from-[#000c18] to-[#083543] border border-slate-700 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/80 to-transparent p-4">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-xl font-bold font-parisienne bg-gradient-to-r from-[#785730] to-[#e9d079] bg-clip-text text-transparent">{treeData.name}</h2>
        ...
        <div className="w-full h-full">
          ...
          >
            <Suspense fallback={<LoadingSpinner />}>
              ...
              {/* Tree Model */}
              <TreeModel url={treeData.model} name={treeData.name} />
          ...
          <div className="bg-black/70 backdrop-blur-sm p-3">
            <div className="flex justify-between items-center text-white text-sm">
              <div>
                ...
                <span className="ml-2">🖱️ Drag to rotate • 🔍 Scroll to zoom • ⚡ Right-click to pan</span>
              </div>
              <button
                onClick={onClose}
