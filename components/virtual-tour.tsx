...
import TreeInventoryModal from './tree-inventory-modal';
...
export default function VirtualTour({
  ...
  const [showTreeInventory, setShowTreeInventory] = useState(false);
  ...
  const handleMarkerClick = (e: { marker: { data?: { nodeId?: string; type?: string; title?: string; text?: string } } }) => {
    ...
    if (marker.data?.nodeId) {
      ...
      // Check if it's the trees info spot
      if (marker.data.title === 'Beautiful Trees') {
        setShowTreeInventory(true);
      } else {
        // Show regular info popup for other spots
        alert(`${marker.data.title}\n\n${marker.data.text}`);
      }
    }
  };
    ...
    <div className={`relative ${className}`} style={{ height, width }}>
      ...
      <div className="absolute top-4 left-4 z-10 bg-black/70 text-white px-4 py-2 rounded-lg">
        <h3 className="font-semibold">
          {TOUR_DATA.scenes.find(scene => scene.id === currentScene)?.name}
        </h3>
