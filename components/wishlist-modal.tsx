                ...
                <div className="pt-20 lg:pt-24 h-full overflow-y-auto p-4 lg:p-6 custom-scrollbar">
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-white mb-2">{t('wishlist.empty')}</h3>
                            <p className="text-gray-400">{t('wishlist.emptyDescription')}</p>
                        </div>
                    ) : (
                                ...
                                <div key={item.id} className="bg-morpheus-blue-dark/30 backdrop-blur-sm rounded-lg p-4 border border-morpheus-gold-dark/20">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => item.yproduit_id && handleMoveToCart(item.yproduit_id)}
                                                className="flex-1 bg-gradient-to-r from-morpheus-gold-dark to-morpheus-gold-light text-white py-2 px-4 font-semibold rounded-lg shadow-lg hover:shadow-morpheus-gold-light/30 transition-all duration-300 text-sm"
                                            >
                                                {t('wishlist.moveToCart')}
                                            </button>
                                            <button
                                                onClick={() => item.yproduit_id && removeFromWishlist(item.yproduit_id)}
                                                className="w-10 h-10 rounded-lg border-2 border-red-500/30 text-red-400 hover:border-red-500 hover:text-red-300 hover:bg-red-500/10 transition-all duration-300 flex items-center justify-center"
                                            >
